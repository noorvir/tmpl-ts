...
function createOverlayWindow(): void {
  ...
  if (overlayWindow && !overlayWindow.isDestroyed()) {
    console.log('Overlay window already exists and is not destroyed, skipping creation')
    return
  }
  ...
  try {
    ...
    const sendStart = () => {
      if (overlayWindow && !overlayWindow.isDestroyed()) {
        ...
        console.error('Cannot send overlay:start - overlay window is destroyed or null')
        isRecording = false
      }
    }
...
function hideOverlay() {
  ...
  try {
    if (overlayWindow && !overlayWindow.isDestroyed()) {
      ...
      if (overlayPendingStartHandler) {
        if (overlayWindow.webContents.isLoadingMainFrame()) {
          overlayWindow.webContents.removeListener('did-finish-load', overlayPendingStartHandler)
        }
        overlayPendingStartHandler = null
      }
...
function startHealthCheck() {
  ...
  healthCheckInterval = setInterval(() => {
    ...
    if (overlayWindow && overlayWindow.isDestroyed() && isRecording) {
      console.warn('Health check: Overlay window is destroyed but recording state is true, resetting')
      isRecording = false
      overlayWindow = null
      previousFocusedWindow = null
    }
    ...
    if (overlayWindow && !overlayWindow.isDestroyed()) {
      try {
        ...
        console.warn('Health check: Overlay window is unresponsive, cleaning up')
        isRecording = false
        destroyOverlay()
      }
    }
    ...
    const success = globalShortcut.register(accelerator, () => {
      try {
        ...
        if (!isRecording) {
          ...
          if (overlayWindow && overlayWindow.isDestroyed()) {
            console.log('Overlay window is destroyed but reference exists, cleaning up')
            overlayWindow = null
          }
          ...
          setTimeout(() => {
            if (!isRecording && overlayWindow && !overlayWindow.isVisible()) {
              console.log('Destroying hidden overlay window to prevent accumulation')
              destroyOverlay()
            }
          }, 2000)
        }
